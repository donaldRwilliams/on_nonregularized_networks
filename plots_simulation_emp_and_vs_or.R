...
sim_results_accuracy <- sim_results %>%
  filter(measure %in% c("cor", "mse"),
         model %in% c("bic_and", "bic_or", "aic_and", "aic_or"))
...
summary_accuracy$model <- factor(summary_accuracy$model,
                                 levels = c("aic_or", "aic_and", "bic_or", "bic_and"),
                                 labels = c("AIC (or)", "AIC (and)", "BIC (or)", "BIC (and)"))
...
leg_plt <- summary_accuracy %>% filter(measure == "Correlation", p == "p = 20") %>%
  ...
  ylab("Correlation") +
  xlab("") +
  scale_fill_manual(values = c("#D55E00","#E69F00", "#000000", "#999999"),
                    name = "Model") +
